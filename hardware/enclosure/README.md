...
harness/           ハーネス本体
  ...
  underside.py     基板の下に出ている実装部品と受け面の隙間
  checks/          13 のチェック（下記）
...
docs/pcb-tray.md        基板トレー。CSI レスキューを付けた Pi と x の収支 0.4 mm
```
...
| 12 | underside | 基板の**下**に出ている実装部品が受け面に当たらないか（同じく形から実測） |
...
- 蓋の締結点を四隅 4 点に戻した変種 → `seal` が FAIL（実際に出た指摘の再現）
- トレーの座面を縮めた変種 → `underside` が FAIL（基板の裏のナットが当たる）
