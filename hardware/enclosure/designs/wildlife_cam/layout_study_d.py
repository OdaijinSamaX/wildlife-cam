"""レイアウト検討 案D — 総縦置き（幹が細い前提で幅を最小化する）
...
## 前提
  ...
  - 指示で確定した貫通は **グランド / PIR / ベントの 3 つ**。ただし
    **カメラ窓を 4 つ目として置いている**（カメラが外を見るには開口が要るため）。
    ...
    ここは確認が要る
  - 取り付け方法は未決定。幹に沿う縦長なので、鞍とベルト溝の効きが最も良い。ベルトを何重にも巻く方式が素直。
...
def _layout(p=PARAMS) -> Layout:
    # 幅を最小化する。剛体ブロックは縦、Pi は横向きのまま上に載せる。
    # FPC は Pi のある +Z 側へ出す。レンズは基板中心から FPC 側へ 2.45 mm
    # 寄っている（実測）ので、**窓はレンズ中心に合わせる**（基板中心ではない）。
    _cam = box("cam", (52.0, 6.3, 72.0), "xzy",
               note="レンズは基板中心から FPC 側へ 2.45 mm 寄る（実測）")
    _win = cam_window_center(_cam, "+Z")
    boxes = onyx_assembly((18.0, 6.6, 115.0), "-Z", "X") + [
        box("pi", (32.5, 5.1, 145.0), "xzy", note="横向きのまま上に載せる。幅は 65 で収まる"),
        box("otg_micro", (46.8, 3.4, 114.6), "zxy",
            note="データ口から -Z に 30.8。Onyx の脇を通る"),
        box("pir", (32.0, 11.35, 191.0), "xzy", note="前壁を貫くキャリア φ52 込み"),
        _cam,
    ]
    routes = [{
        "name": "otg_flex",
        "radius": otg_cable.CABLE_DIA / 2 + 2.0,
        "points": [(46.8, 3.4, 99.2), (40.0, 5.0, 104.0), (26.0, 6.4, 112.0),
                   (18.0, 6.6, 118.0)],
    }]
    holes = [
        Hole("gland", "-Z", 20.0, 20.0, 12.6, "ケーブルグランド PG7（電源）"),
        Hole("pir", "-Y", 32.0, 191.0, 26.0, "PIR 貫通口"),
        Hole("vent", "-Z", 50.0, 20.0, 12.3, "防水通気ベント M12"),
        Hole("cam_window", "-Y", _win[0], _win[2], 16.0,
             "カメラ窓（指示の 3 つには数えられていない）"),
    ]
