...
箱の間は 5m 10芯ケーブル 1 本で結び、**使うのは 2 芯だけ**です。

| 芯 | Pi 箱側 | IR 箱側 |
|----|---------|---------|
| 芯① | Pin 12 (GPIO18) | 端子台 → 100Ω → 2SK4017 ゲート |
| 芯② | Pin 14 (GND) | 端子台 → GND バー |

- 12V・約 0.5A のループは IR 箱の中だけで閉じます。ケーブルに流れるのはゲート駆動と GND 基準だけで実質 0mA なので、GND を複数芯で並列にする必要はありません
- 2 芯は隣り合う芯を選んでください（ループ面積を小さくして LTE のノイズを拾わないため）
- 残り 8 芯は切らずに、両端を 1 本ずつ絶縁して予備に残します。断線しても端子台で挿し替えるだけで復旧できます
...
- 箱への引き込みは PG7 グランド貫通のみとし、屋外コネクタは使いません。分離が必要な接続は箱の中の端子台で行います
