...
ヘッダから取るのは以下の 5 本だけです。物理ピン番号は Pi 5 と Pi Zero 2 W で共通です（ver.β の実機は Pi Zero 2 W）。
...
PIR と IR で GND を分けているのは、PIR がヘッダ上段（9・11 番の隣り合わせ）で完結し、IR 側は箱間ケーブルへ独立して引き出すためです。カメラは CSI リボン、Pi の電源は PWR IN の micro-USB なので、どちらもヘッダは使いません。
...
#### 2箱構成と箱間ケーブル（ver.β 現地構成）

投光器がカメラの画角に入ると露出が光源に合って被写体が真っ黒になるため、筐体を 2 つに分けて木の杭へ上下に配置します。上段（IR 箱）に IR 投光器・電池BOX・MOSFET モジュール、下段（Pi 箱）に Pi Zero 2 W・カメラ・PIR・モバイルバッテリー・LTE モデムを入れます。

箱の間は 5m 10芯ケーブル **2 本**で結び、**1 本まるごとを 1 つの線に充てます**（芯を選び分ける手間をなくすための割り切り）。

| ケーブル | Pi 箱側 | IR 箱側 |
|----------|---------|---------|
| ① IR 制御 | Pin 12 (GPIO18) | 端子台 → 100Ω → 2SK4017 ゲート |
| ② IR GND | Pin 14 (GND) | 端子台 → GND バー |

- 12V・約 0.5A のループは IR 箱の中だけで閉じます。ケーブルに流れるのはゲート駆動と GND 基準だけで実質 0mA なので、1 本あたり 1 芯で足ります
- 各ケーブルは 1 芯だけ端子台へ落とし、残り 9 芯はまとめて絶縁しておきます。断線しても現地で挿し替えるだけで復旧できます（10 芯まとめて撚って 1 本の導体として使うこともできますが、その場合は予備芯が残りません）
- **2 本は結束バンドで束ねてください。** 行きと帰りが別ケーブルになるとループ面積が大きくなり、LTE のノイズを拾いやすくなります
- **両端にタグを貼って ①② を区別してください。** 取り違えると GPIO18 が GND バーへ直結し、点灯指示のたびに GPIO を短絡させます。「ケーブル① ↔ ゲート = 100Ω」の導通チェックで必ず検出できます
- **100Ω と 10kΩ は必ず IR 箱側（MOSFET と同じモジュール）に置いてください。** ケーブルが断線してもゲートは 10kΩ でソースに引き落とされたままなので、IR は消灯側に倒れます。10kΩ を Pi 側へ移すとこのフェイルセーフが失われます
- 箱への引き込みは PG7 グランド貫通のみとし、屋外コネクタは使いません。分離が必要な接続は箱の中の端子台で行います。グランド穴は各箱 2 個です
