...
CadQuery で形を書き、多視点レンダと断面図を自動生成し、**12 種類の数値チェック**（壁厚、干渉、クリアランス、オーバーハング、開口、パッキン面圧、視野、レイアウト、多様体性、外形、嵌合、捕捉式ねじ）を通してから造形します。AI に CAD を書かせるときの最大の弱点は「目が無い」ことなので、**意図的に壊したモデルで各チェックが FAIL することを確かめるネガティブテスト**を必須にしてあります。
