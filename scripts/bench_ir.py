...
現地(ver.β)は 2箱構成で、箱間は 10芯ケーブル2本を1本ずつ充てる:
ケーブル① = GPIO18(物理12番)、ケーブル② = GND(物理14番)。
100Ω と 10kΩ は IR 箱側のモジュールに置くので、ケーブルが断線しても
ゲートはプルダウンされたまま = IR は消灯側に倒れる。
①② を取り違えると GPIO18 が GND バーへ直結するので、通電前に
「ケーブル① ↔ ゲート = 100Ω」を必ず当たること。
PIR の GND は物理9番で、この2本とは別系統。
