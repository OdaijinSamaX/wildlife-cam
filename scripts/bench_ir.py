...
現地(ver.β)は 2箱構成で、箱間の 10芯ケーブルは 2芯しか使わない:
GPIO18(物理12番) と GND(物理14番)。100Ω と 10kΩ は IR 箱側の
モジュールに置くので、ケーブルが断線してもゲートはプルダウンされたまま
= IR は消灯側に倒れる。PIR の GND は物理9番で、この2芯とは別系統。
