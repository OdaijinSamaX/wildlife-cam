...
この姿勢だと **造形時の Z は箱の「奥行き」= 47 mm** になる。設計上の高さ 180 mm は
...
それでも高さは減らした。**229 -> 180 mm。** 理由は造形ではなく取り付け:

  - 受風面積が減る（84 x 229 = 192 cm2 -> 84 x 180 = 151 cm2、-21%）
  - 材料と質量が減る
  - 余裕は安全のためにある（指示のとおり）
...
| 受風面積 | 176 cm2 | 151 cm2 | -14%（高さを減らしたぶん） |
...
## 優先順位（指示のとおり）
  ...
  1. 高さを超えない -> 180 mm（上限 229 に対し 49 mm の余裕）
  2. 張り出しを増やさない -> +3.5 mm。ドームの口径で決まっており、これが下限
  3. 体積 -> 84 x 47 x 180 = 710 cm3（蓋とドームを除く）
...
PARAMS = {
    ...
    "lid_boss_dia": 8.2,
    "lid_boss_depth": 8.0,
    # 四隅。部品を避けた位置で、上下・左右に均等に効かせる。
    # 四隅。柱は背面の land に接する位置に置くこと（浮いた柱は造形できない）。
    # 柱は前壁から背面 land まで通す（浮かないし、箱のねじれにも効く）。
    # x=31 はパッキン溝（37.65〜40.35）から座ぐりを 2 mm 以上離すため。
    "lid_bosses": ((31.0, 12.0), (-31.0, 12.0), (31.0, 186.0), (-30.0, 186.0)),
    # **ポカヨケ**: 4 本目だけ M5、かつ x を 1 mm ずらしてある。180 度回すと
...
def _shell(p, f):
    ...
    land = cq.Solid.makeBox(
        p["width"] - 2 * WALL, p["rim_t"] + 1, p["height"] - 2 * WALL,
